...
def run_expression(packet):
	total = 0

	if packet[1]==0:
		total = sum([run_expression(p) for p in packet[2]])
	elif packet[1]==1:
		total = reduce(lambda a,b: a*b, [run_expression(p) for p in packet[2]])
	elif packet[1]==2:
		total = min([run_expression(p) for p in packet[2]])
	elif packet[1]==3:
		total = max([run_expression(p) for p in packet[2]])
	elif packet[1]==4:
		total = packet[2]
	elif packet[1]==5:
		total = 1 if run_expression(packet[2][0]) > run_expression(packet[2][1]) else 0
	elif packet[1]==6:
		total = 1 if run_expression(packet[2][0]) < run_expression(packet[2][1]) else 0
	elif packet[1]==7:
		total = 1 if run_expression(packet[2][0]) == run_expression(packet[2][1]) else 0
...
print(run_expression(packets[0]))